...
![Taylor Swift](../data/clean/images/5ybJm6GczjQOgTqmJ0BomP.png){#fig-tay}
![Sabrina Carpenter](../data/clean/images/0lBwoEOqZNwm07Pd8Hwj3L.png){#fig-carpenter}
![AC/DC](../data/clean/images/2d4e45fmUnguxh6yqC7gNT.png){#fig-acdc}
![ABBA](../data/clean/images/0GjEhVFGZW8afUYGChu3Rr.png){#fig-abba}
...
Several methods can be used for unsupervised learning, among them, K-means clustering, Hierarchical Clustering and PCA were performed, due to somewhat disappointing results for Hierarchical Clustering and PCA, only K-means clustering was kept. Nevertheless, the code for other methods can be found in the Appendix, presented as is, without result analysis.
...
Only numeric variables were chosen to conduct this clustering experiment, and more specifically only track features were selected, these include: **danceability, energy, loudness, mode, speechiness, acousticness, instrumentalness, liveness, valence, tempo, time_signature**. The decision to only take up an interest in these variables is because these were created by spotify, who does not release any information regarding these features and the calculation behind. One lead may be that these features are caclulated through some sort of clustering to help understand different features and music genres trough it. A clustering analysis of tracks is therefore quite fitting.

The first step in clustering is to determine the optimal number of clusters. The metric used to choose the number of clusters is Total within-cluster sum of squares (TWSS). An elbow must be found, which will determine the choice of clusters.
...
cm <- confusionMatrix(predict(rf_model, newdata = test), test$decade)
...
The median song was release in 2007. We are going to perform a random forest on whether the song was release before or after this year.
...
Looking at the release year, some patterns emerge. In terms of count, clusters **1** and **9** contain the most observations.
