...
editor: 
  markdown: 
    wrap: sentence
abstract: |This project explores how popular music has evolved over the past seventy years by analyzing 10,000 top-charting songs from the ARIA and Billboard rankings, covering the period from the 1950s to 2024. Using musical and metadata features provided by Spotify, the study applies both supervised and unsupervised machine learning techniques to understand the key characteristics that define different musical eras and to assess whether these features can predict when a song was released.
Through models like Random Forests and Neural Networks, the research shows that it is possible to distinguish songs from different time periods with moderate accuracy, especially when grouping them into broader categories such as before or after 2007. A combined model using both audio data and album artwork yielded a prediction error of about 7.8 years on average. Clustering techniques and dimensionality reduction were also applied to uncover patterns and stylistic similarities across decades.
The results highlight how musical features available through streaming platforms can reflect broader cultural and production trends. While predicting the exact release year of a song remains complex, this study demonstrates the value of machine learning as a tool for analyzing musical history and trends, offering useful insights for researchers, the music industry, and digital media platforms.
---
...
The analysis of popular music through computational methods offers a unique lens into the cultural, technological, and artistic shifts that have defined society across decades. With the proliferation of digital streaming platforms and the availability of rich, structured audio datasets, the intersection of music and data science has emerged as a fertile ground for scholarly exploration. This study leverages machine learning techniques to analyze 10,000 top-charting songs from ARIA and Billboard rankings, spanning over seventy years, to investigate temporal patterns and predictive markers in musical composition.

Spotify, as the leading global music streaming platform, is not only a primary source of standardized audio and metadata features but also a pivotal player in shaping contemporary listening habits. Its API offers detailed descriptors such as tempo, danceability, energy, valence, and acousticness, which reflect both perceptual and technical properties of recorded music (Spotify for Developers, 2024). These features enable large-scale, reproducible analysis of musical trends across time. Beyond its role as a data provider, Spotify functions as an active cultural intermediary. Its proprietary popularity score—a dynamic measure influenced by recent streaming activity, user interactions, and playlist placements—does not merely reflect public interest but contributes to shaping it through algorithmic recommendations (Eriksson et al., 2019). This self-reinforcing feedback loop positions Spotify as a gatekeeper in the music industry, influencing not only what is heard but what is produced. As a result, understanding how musical features correlate with time-based patterns also provides insights into broader shifts driven by the platform economy.Spotify’s business model, centered on user engagement and advertising or subscription-based monetization, incentivizes behaviors that are increasingly mirrored in music production itself. Empirical studies have noted a trend toward shorter songs, quicker hooks, and higher replay value, aligned with Spotify’s payout structure that rewards full plays over partial ones (Aguiar et al., 2018). This suggests a convergence between data-informed curation algorithms and creative decisions in the studio, making it essential to consider both technological and economic contexts when analyzing musical evolution. Moreover, the inclusion of album artwork as a predictive input in this study reflects Spotify’s visual turn—where cover aesthetics influence playlist inclusion, click-through rates, and perceived genre alignment (Seaver, 2022). By incorporating both auditory and visual data, this project recognizes the multimodal nature of contemporary music consumption.

Through a combination of supervised and unsupervised machine learning methods, this study not only examines the historical trajectory of popular music but also assesses the extent to which Spotify-derived features capture meaningful temporal signals. In doing so, it contributes to computational musicology while offering industry-relevant insights into how platform-mediated dynamics shape musical production, distribution, and reception in the digital age.
...
Some patterns seem to emerge from this clustering. Cluster 9 appears to have lower values for mode compared to others. Cluster 4 has higher speechiness, while clusters 5 and 6 show slightly more acousticness. Cluster 5 also has much higher instrumentalness, and cluster 7 has a significantly lower time signature than the rest.
...
While this graph is interesting, it does not account for the overall distribution of genres. For instance, *Pop* and *Dance Pop* are by far the most present genres in the dataset with respectively *2660* and *1655* instances. To address this, we compute the relative proportion of each genre $rel_in_cluster = cluster_n / total_n$, for example, pop appears 599 times in Cluster 1 out of 2660 total appearances. Only genres with at least 500 appearances are considered to focus only on main genres instead of niche ones.
...
Aguiar, L., & Waldfogel, J. (2018).  *Platforms, promotion, and product discovery: Evidence from Spotify playlists* (NBER Working Paper No. 24713). National Bureau of Economic Research.  https://www.nber.org/papers/w24713
...
Hesmondhalgh, D., Campos Valverde, R., Valdovinos Kaye, D. B., & Li, Z. (2023).  *The impact of algorithmically driven recommendation systems on music consumption and production: A literature review*. Centre for Data Ethics and Innovation.

House of Commons Digital, Culture, Media and Sport Committee. (2021).  *Economics of music streaming: Second report of session 2021–22 (HC 50)*. UK Parliament.
