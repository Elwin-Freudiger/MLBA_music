...
-   To what extent can we predict the release period of a song using supervised machine learning models based on its audio and metadata features?

-   What are the most influential audio characteristics that define popular music trends over time, and how have these features evolved across decades?

-   Can unsupervised learning techniques (e.g., clustering, PCA) uncover meaningful groupings or latent structures in the dataset that align with musical genres, popularity, or eras?
...
The explicit tag, has a very imbalanced classes with 95% of songs being not is_explicit. The popularity of songs are not normally distributed as one may expect. with a mean popularity of 33.
